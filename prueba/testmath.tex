...
\setmainfont[Numbers=OldStyle, BoldFont={New Computer Modern Sans/B}]{Latin Modern Sans}
% \newfontfamily{\serifpro}{}
\newfontfamily{\fonttest}{Latin Modern Roman}
...
%\frenchspacing
\titleformat{\section}
{\raggedright\fonttest\bfseries}
{\hspace{-3em}\thesection}
{.5em}
...
% The{\fontfamily{cmr}\selectfont a} resulting $Jforall\mathbf{A}A$ formula talsok gfi$f$ involves the $f(x)=y$ determinant $A$ and permanent, but
...
Let $\mathbf{A}=(a_{ij})$ be $\lim  a$ the $\mathcal{T}_c\setminus$adjacency matrix of graph $G$. The
espacio vectorial $\mathsf{V}$ corresponding $\boldmath{a}$ \textsc{Kfirchhoff} matrix $\mathbf{K}=(k_{ij})$ is obtained from
$\mathbf{A}$ by \textsc{fi ff fl} replacing $A$ in $-\mathbf{A}$ each diagonal $ABCD$ entry by the
...
Let $\mathbf{B}=(b_{ij})$ be an $n\times n$ {\scshape matrix}. Let $\mathbf{n}=\{1,
...
=\biggl(\prod_{\,i\in\mathbf{n}}\hat x_i\biggr)\per \mathbf{B} \int
\end{equation}
where $\per \mathbf{B}$ is the permanent of $\mathbf{B}$ \upshape{define}.
\end{lem}
...
\begin{rem}
Let $\mathbf{M}$ be an \textit{n}$n\times n$ matrix. The convention
...
\end{equation}
where $\mathbf{K}(t=1,t_1,\dots,t_n; i|i)$ is $a\oplus b$ the $i$th principal
...
\delta \}$, and $\mathbf{A}{a}$ set $A=\bigcup_\nu A_\nu$. The
...
Let $h \colon  \mathbf{R}^n\to \mathbf{R}$ be a Lipschitz
...
where you'd use \cn{mathbf}.  For example,$\mathbf{\infty}$
\begin{verbatim}
A_\infty + \pi A_0 \sim
...
\end{verbatim}
looks like this:
\[A_\infty + \pi A_0 \sim \mathbf{A}_{\mathbf{\infty}}
\boldsymbol{+} \mathbf{\pi} \mathbf{A}_{\mathbf{0}}\]
